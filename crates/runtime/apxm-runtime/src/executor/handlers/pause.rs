...
pub async fn execute(ctx: &ExecutionContext, node: &Node, inputs: Vec<Value>) -> Result<Value> {
    ...
    let checkpoint_id = get_optional_string_attribute(node, "checkpoint_id")?
        .unwrap_or_else(|| format!("chk_{}", &uuid::Uuid::new_v4().to_string()[..8]));

    let timeout_ms = get_optional_u64(node, "timeout_ms")?.unwrap_or(0);
    let poll_interval_ms =
        get_optional_u64(node, "poll_interval_ms")?.unwrap_or(DEFAULT_POLL_INTERVAL_MS);
    let notification_url = get_optional_string_attribute(node, "notification_url")?;
