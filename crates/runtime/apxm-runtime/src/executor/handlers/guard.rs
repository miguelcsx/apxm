...
pub async fn execute(ctx: &ExecutionContext, node: &Node, inputs: Vec<Value>) -> Result<Value> {
    ...
    let on_fail = get_optional_string_attribute(node, "on_fail")?
        .unwrap_or_else(|| "halt".to_string());
    let error_message = get_optional_string_attribute(node, "error_message")?
...
mod tests {
    ...
    fn test_guard_numeric_gt() {
        assert!(evaluate_condition("> 0.8", &Value::Number(Number::Float(0.9))));
        assert!(!evaluate_condition("> 0.8", &Value::Number(Number::Float(0.7))));
    }

    #[test]
    fn test_guard_numeric_int() {
        assert!(evaluate_condition(">= 5", &Value::Number(Number::Integer(5))));
        assert!(!evaluate_condition("> 5", &Value::Number(Number::Integer(5))));
        assert!(evaluate_condition("< 10", &Value::Number(Number::Integer(3))));
    }

    #[test]
    fn test_guard_not_null() {
        assert!(evaluate_condition("not_null", &Value::String("hello".to_string())));
        assert!(!evaluate_condition("not_null", &Value::Null));
    }

    #[test]
    fn test_guard_not_empty() {
        assert!(evaluate_condition("not_empty", &Value::String("hi".to_string())));
        assert!(!evaluate_condition("not_empty", &Value::String(String::new())));
        assert!(!evaluate_condition("not_empty", &Value::Null));
    }

    #[test]
    fn test_guard_string_eq() {
        assert!(evaluate_condition("== approved", &Value::String("approved".to_string())));
        assert!(!evaluate_condition("== approved", &Value::String("rejected".to_string())));
    }

    #[test]
    fn test_guard_is_null() {
        assert!(evaluate_condition("is_null", &Value::Null));
        assert!(!evaluate_condition("is_null", &Value::String("x".to_string())));
    }
}
