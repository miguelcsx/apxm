...
use super::{ExecutionContext, Node, Result, Value, get_optional_string_attribute, get_optional_u64_attribute, get_string_attribute};
use crate::aam::{Goal, GoalId, GoalStatus, TransitionLabel};

pub async fn execute(ctx: &ExecutionContext, node: &Node, inputs: Vec<Value>) -> Result<Value> {
    let action = get_optional_string_attribute(node, "action")?
    ...
    match action.to_lowercase().as_str() {
        ...
        "remove" => {
            ...
            let goal_id_str = get_string_attribute(node, "goal_id")?;
        ...
        _ => {
            let goal_id_str = get_string_attribute(node, "goal_id")?;
            let priority = get_optional_u64_attribute(node, "priority")?.unwrap_or(1) as u32;
